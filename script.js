...
$(function(){
    ...
    $("body").keydown(function(e){
        if(e.which == 13 && started == false){
            started = true;
            $(".afterStart").css("display", "inherit");

            $("#startMessage").css("display", "none");

            $(".displayedText").html(displayWord)
            timer = setInterval(function() {
                count--
                $("#countdown").html(count);
                if(count == 0){
                    $("#wpm").text("Words per minute: " + goodWordCounter);
                    clearInterval(timer);
                }     
            }, 1000);        
        }
        ...
        if(count > 0 && e.which != 13 && started == true){
            temp += e.key;
            $("#inputP").text(temp);
            if(!temp.match(displayWord)){
                $("#inputP").css("color", "red");
                if(temp.length == displayWord.length){
                    temp = ""; 
                    ranNum = parseInt(Math.random()*1000);
                    displayWord = arrayOfWords[ranNum].toLowerCase();
                    $(".displayedText").html(displayWord)
                    wordsShown++;
                    badWordCounter++;
                    $("#badWords").text("Number of bad words: " + badWordCounter);
                }
            } else {
                $("#inputP").css("color", "rgb(153, 255, 0)");
                    temp = ""; 
                    ranNum = parseInt(Math.random()*1000);
                    displayWord = arrayOfWords[ranNum].toLowerCase();
                    $(".displayedText").html(displayWord)
    ...
    $("body").keydown(function(e){
        if(e.which == 8){
            if(newTemp != "" || temp != ""){
                newTemp = temp.slice(0, temp.length -1);
                $("#inputP").text(newTemp);
                temp = newTemp
                correctionCounter++;
                $("#corrections").text("Number of corrections: " + correctionCounter); 
            }
        }
    })
